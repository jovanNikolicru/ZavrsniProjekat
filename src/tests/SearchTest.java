...
public class SearchTest extends BasicTest {
	...
	public void searchTest() throws InterruptedException, IOException {
		...
		for(int i =1;i<7;i++) {
			...
			sa.assertTrue(actualNoOfResults==noOfResultsInData,"[ERROR] Number of search results didn't match");
			
			for(int j=0;j<srp.listOfMeals().size();j++) {
				
				String namesInData=sheet.getRow(i).getCell(3+j).getStringCellValue();
				sa.assertTrue(srp.listOfMeals().get(j).getText().contains(namesInData),"[ERROR] Names don't match");
			}
		}
		sa.assertAll();
	}
